...
ELSE IF $ARGUMENTS가 "search " 또는 "검색 "으로 시작하면 (대소문자 무시)
  ...
  → WebSearch 호출: WebSearch("<주제>") (도구 미사용 환경이면 폴백으로 직행)
    ...
    - 성공 시 SEARCH_DATA = 핵심 3~5개 포인트로 압축한 검색 결과 (총 500자 이내). 명시적 할당: `SEARCH_DATA = "<압축 결과>"`
  → "Deep 모드 실행" 섹션으로 이동
ELSE
  → 주제 = $ARGUMENTS 전체
  → SEARCH_DATA = "없음"
...
- **폴백**: 실패 또는 도구 미사용 시 SEARCH_DATA = "검색 실패", SEARCH_MODE = "검색 실패 — 내부 모듈만 사용"
- **성공 시**: 명시적 할당 — `SEARCH_DATA = "<핵심 3~5개 포인트 압축, 500자 이내>"`, `SEARCH_MODE = "search 모드 — WebSearch 사용"`
